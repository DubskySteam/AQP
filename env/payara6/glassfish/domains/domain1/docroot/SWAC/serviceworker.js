...
self.importScripts(SWAC.config.swac_root + '/swac/libs/localforage/localforage.min.js');
...
//    SWAC.config.coreComponents.push(SWAC.config.swac_root + "/swac/swac.js");
//    // Add localforage script used by serviceworker
//    SWAC.config.coreComponents.push(SWAC.config.swac_root + '/swac/libs/localforage/localforage.min.js');
//    // Add default pages
//    SWAC.config.coreComponents.push(SWAC.config.app_root + "/js/offline.js");
//    SWAC.config.coreComponents.push(SWAC.config.app_root + "/sites/offline.html");
//    SWAC.config.coreComponents.push(SWAC.config.swac_root + "/swac/offline/offline.json");

    // Install used components
    //self.importScripts(SWAC.config.swac_root + "/swac/Component.js");
    //for (let i in SWAC.config.progressive.components) {
    //    precacheComponent(SWAC.config.progressive.components[i]);
    //}

    //evt.waitUntil(precache(SWAC.config.coreComponents));
...
function precacheComponent(componentName) {
    let component_root = SWAC.config.swac_root + '/swac/components/'
            + componentName + '/';
...
self.addEventListener('activate', function (evt) {
    ...
    evt.waitUntil(precache(SWAC.config.progressive.precache));

    // Create timeout date
    let timeoutTime = new Date().getTime() - (SWAC.config.progressive.cachetimeout * 24 * 60 * 60 * 1000);
...
self.addEventListener('fetch', function (evt) {
    ...
    for (let curSource of SWAC.config.datasources) {
...
function useFallback(request) {
    ...
    if (plainURL.endsWith('.html')) {
        fallbackURL = SWAC.config.app_root + '/sites/offline.html';
        sendMessageToPages('test offline html!');
    } else if (plainURL.endsWith('.js')) {
        fallbackURL = SWAC.config.app_root + "/js/offline.js";
