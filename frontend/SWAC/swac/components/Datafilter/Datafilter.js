...
export default class Datafilter extends View {
    ...
    constructor(options) {
        ...
        this.desc.developers = '';
        this.desc.license = 'GNU Lesser General Public License';
        
        this.desc.depends[0] = {
            name: 'Select',
            path: SWAC.config.swac_root + 'components/Select/Select.js',
        ...
        if (name.length) {
            for (let i = 0; i < filterNameList.length; i++) {
                if (filterNameList[i].value === name) {
                    UIkit.modal.confirm(SWAC.lang.dict.datafilter.confirmupdate).then(function () {
    ...
    checkXAxis(select) {
        ...
        for (let objects in req.data) {
            if (typeof req.data[objects].getSet(0)[select.value] === "number") {
    ...
    syncObservedObjectSelection(id) {
        ...
        if (id !== undefined) {
            ...
            fetch(SWAC.config.datasources[0].replace('[fromName]', '') + 'observedobjecttype/get?id=' + id)
                    .then((response) => {
    ...
    buildSet() {
        ...
        let acc_cont;
        if (splitSelect.value !== SWAC.lang.dict.datafilter.pleaseChoose) {
            ...
            refSelAll.innerHTML = SWAC.lang.dict.datafilter.selectall;
        ...
        splitSelect.onchange = function () {
            ...
            for (let i = 0; i < accordion_cont.length; i++) {
                ...
                parent.children[0].innerHTML = SWAC.lang.dict.datafilter.data;
    ...
    loadFilter(filter_id) {
        ...
        fetch(SWAC.config.datasources[0].replace('[fromName]', '') + 'datavisualisation/get?id=' + filter_id)
                .then((response) => {
                    return response.json();
                }).then((filter) => {
            console.log(filter);
    ...
    saveFilterConfiguration() {
        ...
        fetch(SWAC.config.datasources[0].replace('[fromName]', '') + 'observedobjecttype/getByName?name=' + observedObject)
                .then((response) => {
                    return response.json();
                }).then((observedObject) => {
            datavis.observedObjectType = "ref://" + parseInt(observedObject.id);
            fetch(SWAC.config.datasources[0].replace('[fromName]', '') + 'datavisualisation/create', {
                method: "POST",
                headers: {
    ...
    updateFilterConfiguration() {
        ...
        fetch(SWAC.config.datasources[0].replace('[fromName]', '') + 'observedobjecttype/getByName?name=' + observedObject)
            ...
            datavis.observedObjectType = "ref://" + parseInt(observedObject.id);
            fetch(SWAC.config.datasources[0].replace('[fromName]', '') + 'datavisualisation/update', {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json;"
                },
    ...
    applyCurrentSettings() {
        ...
        if (x_axis_dd.options[x_axis_dd.selectedIndex].value === SWAC.lang.dict.datafilter.pleaseChoose) {
        ...
        try {
            ...
            if (this.filters.length !== 0 || this.sets.length !== 0) {
                ...
                for (let fromName in SWAC_chart.data) {
                    for (let objectNr in SWAC_chart.data[fromName].getSets()) {
                        if (SWAC_chart.data[fromName].has(objectNr))
                            count++;
                    }
                }
    ...
    initFilterEntries() {
        ...
        defaultOption.innerHTML = SWAC.lang.dict.datafilter.pleaseChoose;
    ...
    accordionSelectAll(elem) {
        ...
        if (elem.innerHTML === SWAC.lang.dict.dataflter.selectall) {
            req.selectAllElements(parent);
            elem.innerHTML = SWAC.lang.dict.datafilter.unselectall;
        } else if (elem.innerHTML === SWAC.lang.dict.datafilter.unselectall) {
            req.unselectAllElements(parent);
            elem.innerHTML = SWAC.lang.dict.datafilter.selectall;
        }
    }
    ...
    unselectAllElements(elem) {
        var titel = elem.parentNode.children[0];
        titel.innerHTML = SWAC.lang.dict.datafilter.data;
        for (let i = 1; i < elem.children.length; i++) {
    ...
    fillFilterSelection() {
        ...
        fetch(SWAC.config.datasources[0].replace('[fromName]', '') + 'observedobjecttype/getByName?name=' + observedObject)
            ...
            fetch(SWAC.config.datasources[0].replace('[fromName]', '') + 'datavisualisation/findByObservedObjectType?oo_type_id=' + observedObject.id)
                    .then((response) => {
                        return response.json();
                    }).then((filterList) => {
                ...
                for (let x = 0; x < selectList.length; x++) {
                    ...
                    defaultOption.selected = true;
                    defaultOption.innerHTML = SWAC.lang.dict.datafilter.filterselect;
    ...
    addTitle(checkbox) {
        ...
        if (content !== SWAC.lang.dict.datafilter.data && !content.includes("...")) {
        ...
        } else if (content === SWAC.lang.dict.datafilter.data) {
            title.innerHTML = checkbox.value;
        }
    }
    ...
    subTitle(checkbox) {
        ...
        if (!content.includes("...")) {
            if ((content_split.length - 1) === 0) {
                title.innerHTML = SWAC.lang.dict.datafilter.data;
            } else {
    ...
    setPossibleValues(oo_id, oo_type_id, limit, startset) {
        let thisComp = this;
        return new Promise((resolve, reject) => {
            fetch(SWAC.config.datasources[0].replace('[fromName]', '') + 'measurementtype/listForOoType?ootype_id=' + oo_type_id)
                    .then((response) => {
                        return response.json();
                ...
                // Request data
                Model.load(requestor).then(function (response) {
                    if (Object.entries(thisComp.possibleValue).length === 0) {
                        thisComp.possibleValue = response.data.get(0);
                    } else {
                        for (let dataType in response.data.get(0)) {
                            if (thisComp.possibleValue[dataType] === undefined) {
                                thisComp.possibleValue[dataType] = response.data.get(0)[dataType];
                            } else {
                                for (let data of response.data.get(0)[dataType]) {
