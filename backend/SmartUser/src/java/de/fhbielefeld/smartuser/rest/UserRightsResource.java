...
import de.fhbielefeld.smartuser.rest.proxies.ManageRightInformation;
 ...
 */
@Path("userrights")
@Tag(name = "UserRights", description = "Check and manage user rights")
public class UserRightsResource {
    ...
    @Operation(summary = "Creates a new userright",
            description = "Creates a new userright stored in database")
    @APIResponse(
            responseCode = "200",
            description = "Primary key of the new created userright.",
            content = @Content(
    ...
    public Response create(
            @RequestBody(
                    description = "Username, Resourcestring and Actionname. Optional authtoken of the user that wants to grant right (can also be deliverd over cookie) or apptoken of app wanting to grant right.",
                    required = true) ManageRightInformation createInfo,
            @Context HttpHeaders headers) {

        ResponseObjectBuilder rob = new ResponseObjectBuilder();
        // Check neccessery variables
        if (createInfo.getUsername() == null && createInfo.getAuthtoken() == null) {
            rob.setStatus(Response.Status.BAD_REQUEST);
            rob.addErrorMessage(">username< or >authtoken< are missing.");
            return rob.toResponse();
        }
        if (createInfo.getResource() == null) {
            rob.setStatus(Response.Status.BAD_REQUEST);
            rob.addErrorMessage(">resource< is missing.");
            return rob.toResponse();
        }
        if (createInfo.getAction() == null) {
            rob.setStatus(Response.Status.BAD_REQUEST);
            rob.addErrorMessage(">action< is missing.");
            return rob.toResponse();
        }
        ...
        do {
            ...
            try {
                User requestor = null;
                // Get requesting app
                if (createInfo.getApptoken() != null) {
                    requestor = this.getUserByAuthtoken(createInfo.getApptoken(), um);
                }
                ...
                String username = createInfo.getUsername();
                if (username == null) {
                    username = this.getUserByAuthtoken(createInfo.getAuthtoken(), um).getUsername();
                }

                Message msg = new Message("Try create users right with " + um.getClass().getSimpleName(), MessageLevel.INFO);
                Logger.addDebugMessage(msg);
                um.grantRight(username, createInfo.getResource(), createInfo.getAction(), requestor);
        ...
        if (!reqFound) {
            ...
            rob.addErrorMessage("You have no right to create a right.");
        }
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(summary = "Check access right",
            description = "Checks the right to access a specific resource.")
    @APIResponse(
            responseCode = "200",
            description = "User has access",
            content = @Content(
                    mediaType = "application/json",
                    example = "{\"username\" : [ \" Users Name \"]}"
            ))
    @APIResponse(
            responseCode = "403",
    ...
    @APIResponse(
            ...
            description = "Error mesage",
            content = @Content(mediaType = "application/json",
                    example = "{\"errors\" : [ \" Could not check right: Because of ... \"]}"))
    public Response check(
            ...
            @Parameter(description = "Requestid") @QueryParam("requestid") String requestid,
            @Context HttpHeaders headers) {
        ...
        if (resource == null) {
            rob.setStatus(Response.Status.BAD_REQUEST);
            rob.addErrorMessage("Parameter >resource< is missing");
            return rob.toResponse();
        }
        if (action == null) {
            rob.setStatus(Response.Status.BAD_REQUEST);
            rob.addErrorMessage("Parameter >action< is missing");
            return rob.toResponse();
        }
        ...
        if(reqtoken == null && apptoken == null) {
        ...
        do {
            ...
            if (!umh.isReady()) {
                exceptions.put(um.getClass().getSimpleName(), new UsermanagerException("Usermanager is not ready."));
                continue;
            }
            try {
                User requestor = null;
                ...
                // Get the target username
                if (username == null) {
                    User user = this.getUserByAuthtoken(authtoken, um);
                    if (user == null) {
                        Message msg = new Message("Could not find user for authtoken >" + authtoken + "<", MessageLevel.ERROR);
                        Logger.addDebugMessage(msg);
                        rob.setStatus(Response.Status.UNAUTHORIZED);
                        rob.addErrorMessage("The user you try to check is not logged in.");
                        return rob.toResponse();
                    }
                    username = user.getUsername();
                }

                hasRight = um.userHasRight(username, resource, action);
                if (hasRight) {
                    Message rmsg = new Message("Authentication: User >"
                            + username + "< has the right for resource "
                            + resource + " and action " + action + " useing "
                            + umh.getUsermanager().getClass().getSimpleName(),
                            MessageLevel.INFO);
                    Logger.addDebugMessage(rmsg);
                    break;
                } else {
                    Message nrmsg = new Message("Authentication: User >"
                            + username + "< has NO right for resource "
                            + resource + " and action " + action + " useing "
                            + umh.getUsermanager().getClass().getSimpleName(),
                            MessageLevel.INFO);
                    Logger.addDebugMessage(nrmsg);
                }
            } catch (UsermanagerException | UnsupportedOperationException ex) {
                exceptions.put(um.getClass().getSimpleName(), ex);
                Message failmsg = new Message("AuthenticationFilter: "
                        + "userHasRight(...) failed with: "
                        + ex.getLocalizedMessage(), MessageLevel.ERROR);
                Logger.addDebugMessage(failmsg);
            }
        } while (umh.setNextUsermanagerActive());
        umh.resetActiveUsermanager();
        // Create error on failed requestor right
        if (!reqFound) {
            rob.setStatus(Response.Status.FORBIDDEN);
            rob.addErrorMessage("You have no right to check this right.");
        }
        ...
        if (!hasRight) {
            rob.setStatus(Response.Status.FORBIDDEN);
            rob.addErrorMessage("User >" + username + "< has no right for resource >" + resource + "< and action >" + action + "<");
        }

        long stopTime = System.nanoTime();
        System.out.println("**************************");
        System.out.println("Checking for request " + requestid + " took " + (stopTime - startTime));

        rob.add("username", username);
    ...
    @Path("list")
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(summary = "List rights",
            description = "Lists all rights for the specified user")
    @APIResponse(
            responseCode = "200",
            description = "Richts requested",
            content = @Content(
                    mediaType = "application/json",
                    example = "{\"records\" : [{\"id\" :  1, \"path\" : \"my.path.to.resource.1\", \"action\": \"GET\"}]}"
            ))
    @APIResponse(
            responseCode = "500",
            description = "Error mesage",
            content = @Content(mediaType = "application/json",
                    example = "{\"errors\" : [ \" Could not get rights: Because of ... \"]}"))
    public Response list(
        ...
        if (requestor == null) {
            rob.setStatus(Response.Status.FORBIDDEN);
            rob.addErrorMessage("You have no right to list rights.");
            return rob.toResponse();
        }
        ...
        try {
            userrights = um.userListRights(username, resource, action);
        } catch (UsermanagerException ex) {
            rob.addException(ex);
        ...
        for (UserRight curRight : userrights) {
            ResponseObjectBuilder crob = new ResponseObjectBuilder();
            crob.add("path", curRight.getResource().getPath());
            crob.add("action", curRight.getAction());
            rlb.add(crob);
        }
        rob.add("list", rlb);
        rob.setStatus(Response.Status.OK);
    ...
    @DELETE
    public Response revoke(@Parameter(description = "Username") @QueryParam("username") String username,
            @Parameter(description = "Resource the user want to access") @QueryParam("resource") String resource,
            @Parameter(description = "Action the user want to perform") @QueryParam("action") String action,
            @Parameter(description = "Users authtoken") @QueryParam("authtoken") String authtoken,
            @Parameter(description = "App token") @QueryParam("apptoken") String apptoken,
        ...
        if (requestor == null) {
            rob.setStatus(Response.Status.FORBIDDEN);
            rob.addErrorMessage("You have no right to recoke this right.");
        }
        ...
        Logger.addDebugMessage(msg);
        try {
            // Get user for authtoken
            if(username == null) {
                username = this.getUserByAuthtoken(authtoken, um).getUsername();
            }
            // Revoke right
            um.revokeRight(username, resource, action, requestor);
        } catch (UsermanagerException ex) {
            rob.addException(ex);
            rob.addErrorMessage("Could not get users rights: " + ex.getLocalizedMessage());
            return rob.toResponse();
